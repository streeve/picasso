        ...
        KOKKOS_LAMBDA( const int f ) {
            ...
            for ( int ip = 1; ip <= particles_per_facet_median; ++ip )
            {
                for ( int it = 0; it < 3; ++it )
                {
                    ...
                    Vec3<double> a_median =
                        ( a * ( median_div - ip ) + centroid * ip ) /
                        median_div;
                    Vec3<double> b_median =
                        ( b * ( median_div - ip ) + centroid * ip ) /
                        median_div;
                    Vec3<double> c_median =
                        ( c * ( median_div - ip ) + centroid * ip ) /
                        median_div;
                    Kokkos::Array<Vec3<double>, 3> abc_median = {
                        a_median, b_median, c_median };
